...
def DynamicallyGrownGAN(*args, **kwargs):
    
    from models.dynamically_grown_gan import DynamicallyGrownGAN
    if 'config' not in kwargs or kwargs['config'] is None:
        kwargs['config'] = {}
    
    model = DynamicallyGrownGAN(useGPU=kwargs.get('useGPU', True),
                                storeAVG=True,
                                **kwargs['config'])
    return model